...
// 計時用：在指定的 START_TIME 到達時開始計時，填入 captcha 時停止
let enterStartTime: number | null = null;

// 解析 START_TIME 字串（支援 "YYYY-MM-DD HH:mm:ss" 或 ISO），解析失敗回傳 null
function parseStartTime(input?: string): Date | null {
  if (!input) return null;
  // 如果是簡單的 YYYY-MM-DD HH:mm:ss，轉成 ISO (把空格換成 T)
  let iso = input;
...
function waitUntil(targetTime: Date): Promise<void> {
  return new Promise((resolve, reject) => {
    if (!targetTime || isNaN(targetTime.getTime())) {
      return reject(new Error('waitUntil: 傳入的 targetTime 無效'));
    }
